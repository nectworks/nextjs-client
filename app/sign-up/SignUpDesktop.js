...
const SignUpDesktop = () => {
  ...
  const handleOtpInputChangeSignUp = (index, value) => {
    if (value === '' || /^[0-9]$/.test(value)) {
      ...
      // If the input is empty or a number, update the corresponding OTP value
      if (index === 3) {
        setLoginButtonBg(true);
      }
      if (value !== '' && index < 3) {
        /*
          If the input is a number and not the last input,
        ...
        */
        ...
        /*
          If the input is deleted and not the first input,
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        /* if the user is registered successfully,
           redirect them to the previous location they came from
           or redirect them to the profile page
        */
        navigate(prevLocation || '/profile', {
          state: {
            from: '/sign-up',
  ...
  useEffect(() => {
    ...
    if (user) {
      navigate(prevLocation || '/profile', { replace: true });
      return;
    }
  }, []);
